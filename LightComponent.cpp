...
//{
	...
	//Framebuffer& depthFramebuffer = type == LightType::DIRECTIONAL ? Application::get().getPipeline().getDepthFramebuffer2D() : Application::get().getPipeline().getDepthFramebuffer();
	//depthFramebuffer.clear();
	//depthFramebuffer.bind();

	//float nearplane = 1.f;
	//float farplane = 1 + linear * getDistance() + getDistance() * quadratic * quadratic;

	//glm::mat4 pv = glm::mat4(1);
	//

	//if (type == LightType::DIRECTIONAL) {
	//	glm::mat4 lightProjection = glm::ortho(-10.f, 10.f, -10.f, 10.f, nearplane, 100.f);
	...
	//	glCullFace(GL_FRONT);
	//	Application::get().getPipeline().drawCallEvent(pv, m_directDepthPassShader);
	//	glCullFace(GL_BACK);
	...
	//	m_directLightPassShader->setTexture("depthMap", Application::get().getPipeline().getDepthTexture2D());
	...
	//	glCullFace(GL_FRONT);
	//	Application::get().getPipeline().drawCallEvent(pv, m_depthPassShader);
	//	glCullFace(GL_BACK);
	//	depthFramebuffer.unbind();
	...
	//	m_lightPassShader->setTexture("depthMap", Application::get().getPipeline().getDepthTexture());
